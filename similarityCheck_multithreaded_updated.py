...
def calculate_average_similarity_and_return_all(df1, df2):
    ...
    with Pool() as pool:
        ...
        for i, (id, average_similarity, results) in enumerate(pool.imap(calculate_similarity, args)):
            print(f'Comparing sequence with ID {id} : Average similarity of sequence {sequence_ids+i+1} with all sequences in the second dataset: {average_similarity}')
            pd.DataFrame({'ID': [id], 'Average Similarity': [average_similarity]}).to_csv('average_similarity.csv', mode='a', header=False, index=False)
...
if __name__ == '__main__':
    ...
    data2_path = './data_files\deeploc1.0\deeploc_data.fasta'
